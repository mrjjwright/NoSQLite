(function(){
  var assert, core_data_create_sql, log, predicate, simple_obj, sql, sys, table_sql, table_sql_guid;
  sql = require("../sql");
  ...
  assert.equal(sql.insert("log", {
    text: "hello",
    log_type: "mumble"
  }).index_placeholder, "insert into log(text,log_type) values (?,?)", "insert index_placeholder should be correct");
  assert.equal(sql.insert("log", {
    text: "hello",
    log_type: "mumble"
  }).name_placeholder, "insert into log(text,log_type) values (:text, :log_type)", "insert name_placeholder should be correct");
  assert.equal(sql.insert("log", {
    text: "hello",
    log_type: "mumble"
  }).escaped, "insert into log(text,log_type) values ('hello','mumble')", "insert escaped should be correct");
  assert.equal(sql.insert("log", {
    text: "hello",
    log_type: "mumble"
