(function(){
  ...
  SQL.prototype.select = function(table, predicate) {
    ...
    for (_a = 0, _c = _b.length; _a < _c; _a++) {
      ...
      for (key in _d) { if (__hasProp.call(_d, key)) {
        ...
        this.bindings.push(this.convert_to_sqlite(predicate[key]));
      }}
    }
  ...
  SQL.prototype.insert = function(table, obj, options) {
    var _a, _b, _c, _d, _e, columns_sep, key, names, question_marks, sql;
    sql = "insert ";
    if ((typeof (_a = typeof options === "undefined" || options == undefined ? undefined : options.replace) !== "undefined" && _a !== null) === true) {
      sql = sql + " or replace ";
    }
    if ((typeof (_b = typeof options === "undefined" || options == undefined ? undefined : options.ignore) !== "undefined" && _b !== null) === true) {
      sql = sql + " or ignore ";
    }
    sql = sql + "into " + this.sql_name(table);
    question_marks = [];
    names = [];
    if ((typeof (_c = typeof options === "undefined" || options == undefined ? undefined : options.rowid_sql) !== "undefined" && _c !== null) && (typeof (_d = typeof options === "undefined" || options == undefined ? undefined : options.rowid_name) !== "undefined" && _d !== null)) {
      this.columns.push(options.rowid_name);
      question_marks.push(("(" + (options.rowid_sql) + ")"));
    }
    _e = obj;
    for (key in _e) { if (__hasProp.call(_e, key)) {
      if (_.include(this.columns, this.sql_name(key))) {
        continue;
      }
      if (key === "nsl_children") {
        continue;
      }
      this.bindings.push(this.convert_to_sqlite(obj[key]));
  ...
  SQL.prototype.create_table = function(table, obj, rowid_name) {
    ...
    (typeof rowid_name !== "undefined" && rowid_name !== null) ? this.columns.push(("\"" + (rowid_name) + "\" INTEGER PRIMARY KEY")) : null;
    _a = obj;
    for (key in _a) { if (__hasProp.call(_a, key)) {
      ...
      key === "guid" || key === "uuid" ? (type = "VARCHAR UNIQUE NOT NULL") : null;
  ...
  SQL.prototype.convert_to_sqlite = function(value) {
    ...
    if (_.isNumber(value) === true || _.isString(value) === true) {
      return value;
    }
  ...
  root.sqlite_sql.insert = function(table, obj, options, core_data_mode) {
    return new SQL(core_data_mode).insert(table, obj, options);
  };
  root.sqlite_sql.create_table = function(table, obj, rowid_name, core_data_mode) {
    return new SQL(core_data_mode).create_table(table, obj, rowid_name);
  };
