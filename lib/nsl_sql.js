(function(){
  ...
  nosqlite.sql = {};
  nosqlite.sql.select = function select(table, predicate, core_data_mode) {
    return new SQL(core_data_mode).select(table, predicate);
  };
  nosqlite.sql.insert = function insert(table, obj, replace, core_data_mode) {
    return new SQL(core_data_mode).insert(table, obj, replace);
  };
  nosqlite.sql.create_table = function create_table(table, obj, core_data_mode) {
    return new SQL(core_data_mode).create_table(table, obj);
  };
  nosqlite.sql.add_column = function add_column(table, column, type, core_data_mode) {
    return new SQL(core_data_mode).add_column(table, column, type);
  };
  nosqlite.sql.create_temp_table = function create_temp_table(table, obj, core_data_mode) {
    return new SQL(core_data_mode).create_temp_table(table, obj);
  };
  nosqlite.sql.convert_to_sqlite = function convert_to_sqlite(value, core_data_mode) {
    return new SQL(core_data_mode).convert_to_sqlite(value);
  };
  nosqlite.sql.convert_from_sqlite = function convert_from_sqlite(value, prototype_value, core_data_mode) {
    return new SQL(core_data_mode).convert_from_sqlite(value, prototype_value);
  };
  nosqlite.sql.populate_predicate = function populate_predicate(predicate, obj, core_data_mode) {
